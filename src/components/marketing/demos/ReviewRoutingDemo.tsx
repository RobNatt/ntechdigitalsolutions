...
export function ReviewRoutingDemo() {
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-950 sm:p-8">
      ...
      {outcome === "public" ? (
        <div className="mt-6 rounded-xl border border-emerald-200 bg-emerald-50 p-5 text-center dark:border-emerald-900/50 dark:bg-emerald-950/40">
          <ThumbsUp className="mx-auto h-6 w-6 text-emerald-700 dark:text-emerald-400" aria-hidden />
          ...
          <button
            type="button"
            onClick={reset}
            className="mt-4 text-xs font-semibold text-emerald-800 underline underline-offset-4 hover:text-emerald-950 dark:text-emerald-300"
          >
            Try another rating
          </button>
        </div>
      ...
      {outcome === "private" ? (
        <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-5 dark:border-amber-900/50 dark:bg-amber-950/40">
          {!feedbackSent ? (
            <>
              ...
              <label className="mt-3 block text-xs font-medium text-amber-950 dark:text-amber-100">
                What could we have done better?
                <textarea
                  value={feedback}
                  onChange={(e) => setFeedback(e.target.value)}
                  className="mt-1.5 w-full rounded-lg border border-amber-300 bg-white px-3 py-2 text-sm text-neutral-900 shadow-sm focus:border-amber-500 focus:outline-none dark:border-amber-800 dark:bg-neutral-950 dark:text-neutral-100"
                  rows={2}
                />
              </label>
              <div className="mt-3 flex gap-2">
                ...
                <button
                  type="button"
                  onClick={reset}
                  className="text-xs font-semibold text-amber-800 underline underline-offset-4 hover:text-amber-950 dark:text-amber-300"
                >
                  Try another rating
                </button>
              </div>
            ...
            <>
              ...
              <button
                type="button"
                onClick={reset}
                className="mt-3 text-xs font-semibold text-amber-800 underline underline-offset-4 hover:text-amber-950 dark:text-amber-300"
              >
                Try another rating
              </button>
            </>
