...
export function ChatWidget() {
  ...
  // Written in an effect, not during render. The delayed welcome timeout reads
  // this to check whether chat is already open without having to re-create the
  // timeout on every toggle; mutating a ref mid-render is a tearing hazard
  // under concurrent rendering and the lint rule is right to reject it.
  useEffect(() => {
  ...
  /*
   * Opening chat skips re-showing the welcome card this session.
   *
   * It does NOT call setWelcomeOpen(false) — the render already guards on
   * `welcomeOpen && !open`, so hiding it was derived state kept in sync by
   * hand. Removing the setState both satisfies the set-state-in-effect rule and
   * drops a render nobody needed.
   */
  useEffect(() => {
    if (!open) return;
