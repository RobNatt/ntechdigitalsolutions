...
export function DataCard({
  ...
  return (
    ...
    >
      <div
        className={`h-full rounded-2xl border-2 border-gray-400/40 bg-gradient-to-br from-gray-200/40 via-gray-300/40 to-gray-200/40 backdrop-blur-lg p-6 ${statusGlow[status]} transition-all duration-300 group-hover:border-gray-500/60 relative overflow-hidden`}
      >
        ...
        <motion.div
          ...
          style={{
            background:
              "radial-gradient(circle at center, rgba(150,150,150,0.15) 0%, transparent 70%)",
          }}
          animate={{
            scale: [1, 1.2, 1],
          }}
        ...
        <div className="relative z-10 h-full flex flex-col">
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-400/30">
            <div className="flex items-center gap-2">
              <TrendIcon
                className={`w-4 h-4 ${trend.includes("+") ? "text-gray-700" : trend.includes("-") ? "text-gray-500" : "text-gray-600"}`}
              />
              <span className="text-sm text-gray-700">{trend}</span>
            </div>
            <span
              className={`text-xs px-2 py-1 rounded-full bg-gradient-to-r ${statusColors[status]} bg-opacity-10 text-gray-700 uppercase tracking-wider`}
            >
              {status}
            </span>
          </div>
